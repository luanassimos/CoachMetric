...
export default function EvaluationTemplateEditor({
  ...
  async function loadTemplates(nextTemplateId?: string) {
    ...
    if (!templateIdToLoad && data.length > 0) {
      templateIdToLoad = data.find((item) => item.is_active)?.id ?? data[0].id;
    ...
    try {
      ...
      const nextSort =
        (selectedTemplate.sections?.length ?? 0) > 0
          ? Math.max(...(selectedTemplate.sections ?? []).map((s) => s.sort_order)) + 1
          : 0;
  ...
  if (loading) {
    return <div className="p-6">Loading template editor...</div>;
  }
      ...
      {selectedTemplate ? (
        <>
          <div className="card-elevated p-5 space-y-4">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button onClick={handleSaveTemplateMeta} disabled={savingTemplate}>
          ...
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" onClick={handleAddSection} disabled={creatingSection}>
              <Plus className="mr-1.5 h-4 w-4" />
          ...
          <div className="space-y-4">
            {(selectedTemplate.sections ?? []).length === 0 ? (
              ...
              selectedTemplate.sections
                ...
                .map((section) => (
                  <div key={section.id} className="card-elevated p-5 space-y-4">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        <Button variant="outline" onClick={() => handleSaveSection(section)}>
                          Save Section
                        </Button>
                        ...
                        section.items
                          ...
                          .map((item) => (
                            <div key={item.id} className="rounded-xl border p-4 space-y-4">
                              ...
                              <div className="flex items-center justify-end gap-2">
                                <Button variant="outline" onClick={() => handleSaveItem(item)}>
                                  Save Item
                                </Button>
